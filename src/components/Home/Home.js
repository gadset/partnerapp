...
function Home() {
	...
	useEffect(()=>{
		...
		const resi = await axios.get(process.env.REACT_APP_BACKEND + 'partner/getId', {
        headers: {
	...
	useEffect(() => {
        ...
        const start = async () => {
          try {
            ...
            const bidsdata = await axios.get(process.env.REACT_APP_BACKEND + `partner/getpartnerdata?partnerid=${partnerid}`,
			{
          headers: {
            ...
            setData1([
                ...
                {number: bidsdata.data.delivered,names: "Delivered",link: "/delivery",},
              ]);
          } catch (err) {
            console.log(err);
          }
