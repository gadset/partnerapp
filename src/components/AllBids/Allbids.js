...
export default function Allbids() {
	...
	const partnerid = useSelector((state) => state.partnerid.value) || JSON.parse(localStorage.getItem('partnerid'));
// Now, `partnerid` contains the value you stored in localStorage
    ...
    const getallbids = async () => {
        try {
        const response = await axios.get( process.env.REACT_APP_BACKEND + `users/getquotes`,
		{
          headers: {
    ...
    const handlebidsubmission=async(data)=>{
        ...
        try {
             const resp = await axios.post(process.env.REACT_APP_BACKEND + 'partner/submitquote',{id,partnerid,amount,warranty,service}, {
